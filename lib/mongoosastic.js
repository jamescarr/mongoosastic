...
function createMappingIfNotPresent(client, indexName, typeName, schema, options, cb){
  generator.generateMapping(schema, function(err, mapping){
    ...
    client.indexExists(indexName, function(err, exists) {
      if (exists) {
        ...
        client.createIndex(indexName, {settings: settings, mappings:completeMapping}, cb);
      }
    });
  });
}
